#!/bin/bash

set -e

# bot=chord    # chord, debru, kadem, leet
bot=$1

CUDA_VISIBLE_DEVICES=0 \
python train_botnet.py \
--devid 0 \
--seed 0 \
--in_mem 1 \
\
--data_dir ../../data/botnet/ev10k \
--data_train ${bot}_no100k_ev10k_us_train.hdf5 \
--data_val ${bot}_no100k_ev10k_us_val.hdf5 \
--data_test ${bot}_no100k_ev10k_us_test.hdf5 \
--save_dir ../saved_models/botnet_ev10k \
--save_name ${bot}_lay6_rh1_bs0_att_out_nh1_ep50.pt \
--bsz 2 \
--shuffle 0 \
\
--in_channels 1 \
--enc_sizes 32 32 32 32 32 32 \
--nheads 1 \
--act relu \
--residual_hop 1 \
--nodemodel attention \
--bias 0 \
--dropout 0.0 \
--final proj \
\
--att_dir out \
--att_act lrelu \
--att_dropout 0.0 \
--temperature 0.1 \
--sample 1 \
\
--lr 0.001 \
--weight_decay 5e-4 \
--epochs 50 \
& \
CUDA_VISIBLE_DEVICES=1 \
python train_botnet.py \
--devid 0 \
--seed 0 \
--in_mem 1 \
\
--data_dir ../../data/botnet/ev10k \
--data_train ${bot}_no100k_ev10k_us_train.hdf5 \
--data_val ${bot}_no100k_ev10k_us_val.hdf5 \
--data_test ${bot}_no100k_ev10k_us_test.hdf5 \
--save_dir ../saved_models/botnet_ev10k \
--save_name ${bot}_lay6_rh1_bs1_att_out_nh1_ep50.pt \
--bsz 2 \
--shuffle 0 \
\
--in_channels 1 \
--enc_sizes 32 32 32 32 32 32 \
--nheads 1 \
--act relu \
--residual_hop 1 \
--nodemodel attention \
--bias 1 \
--dropout 0.0 \
--final proj \
\
--att_dir out \
--att_act lrelu \
--att_dropout 0.0 \
--temperature 0.1 \
--sample 1 \
\
--lr 0.001 \
--weight_decay 5e-4 \
--epochs 50 \
& \
CUDA_VISIBLE_DEVICES=2 \
python train_botnet.py \
--devid 0 \
--seed 0 \
--in_mem 1 \
\
--data_dir ../../data/botnet/ev10k \
--data_train ${bot}_no100k_ev10k_us_train.hdf5 \
--data_val ${bot}_no100k_ev10k_us_val.hdf5 \
--data_test ${bot}_no100k_ev10k_us_test.hdf5 \
--save_dir ../saved_models/botnet_ev10k \
--save_name ${bot}_lay4_rh1_bs0_att_out_nh1_ep50.pt \
--bsz 2 \
--shuffle 0 \
\
--in_channels 1 \
--enc_sizes 32 32 32 32 \
--nheads 1 \
--act relu \
--residual_hop 1 \
--nodemodel attention \
--bias 0 \
--dropout 0.0 \
--final proj \
\
--att_dir out \
--att_act lrelu \
--att_dropout 0.0 \
--temperature 0.1 \
--sample 1 \
\
--lr 0.001 \
--weight_decay 5e-4 \
--epochs 50 \
& \
CUDA_VISIBLE_DEVICES=3 \
python train_botnet.py \
--devid 0 \
--seed 0 \
--in_mem 1 \
\
--data_dir ../../data/botnet/ev10k \
--data_train ${bot}_no100k_ev10k_us_train.hdf5 \
--data_val ${bot}_no100k_ev10k_us_val.hdf5 \
--data_test ${bot}_no100k_ev10k_us_test.hdf5 \
--save_dir ../saved_models/botnet_ev10k \
--save_name ${bot}_lay4_rh1_bs1_att_out_nh1_ep50.pt \
--bsz 2 \
--shuffle 0 \
\
--in_channels 1 \
--enc_sizes 32 32 32 32 \
--nheads 1 \
--act relu \
--residual_hop 1 \
--nodemodel attention \
--bias 1 \
--dropout 0.0 \
--final proj \
\
--att_dir out \
--att_act lrelu \
--att_dropout 0.0 \
--temperature 0.1 \
--sample 1 \
\
--lr 0.001 \
--weight_decay 5e-4 \
--epochs 50

exit 0

CUDA_VISIBLE_DEVICES=3 \
python train_botnet.py \
--devid 0 \
--seed 0 \
--in_mem 1 \
\
--data_dir ../../data/botnet/ev10k \
--data_train ${bot}_no100k_ev10k_us_train.hdf5 \
--data_val ${bot}_no100k_ev10k_us_val.hdf5 \
--data_test ${bot}_no100k_ev10k_us_test.hdf5 \
--save_dir ../saved_models/botnet_ev10k \
--save_name ${bot}_lay8_rh1_bs0_att_out_nh1_ep50.pt \
--bsz 2 \
--shuffle 0 \
\
--in_channels 1 \
--enc_sizes 32 32 32 32 32 32 32 32 \
--nheads 1 \
--act relu \
--residual_hop 1 \
--nodemodel attention \
--bias 0 \
--dropout 0.0 \
--final proj \
\
--att_dir out \
--att_act lrelu \
--att_dropout 0.0 \
--temperature 0.1 \
--sample 1 \
\
--lr 0.001 \
--weight_decay 5e-4 \
--epochs 50

CUDA_VISIBLE_DEVICES=3 \
python train_botnet.py \
--devid 0 \
--seed 0 \
--in_mem 1 \
\
--data_dir ../../data/botnet/ev10k \
--data_train ${bot}_no100k_ev10k_us_train.hdf5 \
--data_val ${bot}_no100k_ev10k_us_val.hdf5 \
--data_test ${bot}_no100k_ev10k_us_test.hdf5 \
--save_dir ../saved_models/botnet_ev10k \
--save_name ${bot}_lay8_rh1_bs1_att_out_nh1_ep50.pt \
--bsz 2 \
--shuffle 0 \
\
--in_channels 1 \
--enc_sizes 32 32 32 32 32 32 32 32 \
--nheads 1 \
--act relu \
--residual_hop 1 \
--nodemodel attention \
--bias 1 \
--dropout 0.0 \
--final proj \
\
--att_dir out \
--att_act lrelu \
--att_dropout 0.0 \
--temperature 0.1 \
--sample 1 \
\
--lr 0.001 \
--weight_decay 5e-4 \
--epochs 50
