...
# data_dir = '/n/holylfs/LABS/tata_yu_rush_level1_data/xuzhiying/public_data_processed/chord'
data_dir = '/n/holylfs/LABS/tata_yu_rush_level1_data/TataProject/data_botnet'
data_train_name = 'chord_graphs_udr_slp_train.hdf5'
# data_train_path = os.path.join(data_dir, data_train_name)

data_val_name = 'chord_graphs_udr_slp_val.hdf5'
# data_val_path = os.path.join(data_dir, data_val_name)

data_test_name = 'chord_graphs_udr_slp_test.hdf5'
...
def parse_args():
    ...
    parser.add_argument('--data_train', type=str, default=data_train_name, help='file name of the training dataset')
...
# train_ds = DSTGraphDataset(data_train_path, time_window=1, in_memory=args.in_mem)
# train_loader = DSTGraphLoader(train_ds, batch_size=args.bsz)

val_ds = DSTGraphDataset(data_val_path, time_window=1, in_memory=args.in_mem)
val_loader = DSTGraphLoader(val_ds, batch_size=1)        # for correct calculation of evaluation metrics

# test_ds = DSTGraphDataset(data_test_path, time_window=1, in_memory=args.in_mem)
...
def test(model, data_loader, devid=args.devid):
    ...
    with torch.no_grad():
        for n, batch in enumerate(data_loader):        # here batch size is 1 for val and test data
            ...
            x = model(batch.x, batch.edge_index, norm_method=args.norm_method)
            y = batch.y.long()
            loss = criterion(x, y)
            loss_avg += float(loss)
            pred = x.argmax(dim=1)
...
#        breakpoint()
        ...
        if num_train_graph % 1 == 0:
            with torch.no_grad():
                ...
                rec = recall(pred, y)
                prc = precision(pred, y)
            log(f'epoch: {ep + 1}, passed number of graphs: {num_train_graph}, train running loss: {loss_avg_train / num_train_graph:.5f} (passed time: {timeSince(start)})')
