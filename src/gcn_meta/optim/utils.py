...
def show_cuda_mem(logger=None):
    show = logger.debug if logger else print
    show(f"Cuda Mem (MiB): {torch.cuda.memory_allocated() / (1024 ** 2)}")
    show(f"Cuda Mem Cached (MiB): {torch.cuda.memory_cached() / (1024 ** 2)}")
    show(f"Cuda Mem Max (MiB): {torch.cuda.max_memory_allocated() / (1024 ** 2)}")
    return
